...
def usage():
    print("Usage:")
    print("\tpython3 pb_generator.py [pb_file]")
    print("Input:")
...
def show_solution(sol_out,n,m):
    ...
    print(sol)
    fig, ax = plt.subplots()
    ax.matshow(sol, cmap=plt.cm.gist_ncar_r)
    for i in range(n):
        for j in range(m):
            c = sol[i,j]
...
def main(argc, argv):
    ...
    print("Generating Pseudo Boolean...",end="")
    generate_pseudo_boolean(points, n, m, p, pb_filename, n*m)
    print("done")

    ## Solving with pbsolver
    print("Solving problem with pbsolver...",end="")
    ...
    if not check_solution(proc.stdout,n,m,p): 
        ...
        while(left+1<right):
            ...
            print("Trying cost=%d..."%c_max,end='')
            generate_pseudo_boolean(points, n, m, p, pb_filename, c_max)
            solver = os.path.join(os.path.dirname(os.path.abspath(__file__)),"../BasicPBSolver/pbsolver")
            proc = subprocess.run([solver,pb_filename,'model', "pb_sorter"], stdout=subprocess.PIPE, encoding='ascii')
            if check_solution(proc.stdout,n,m,p):
                print("SAT")
                right=c_max
                right_sol=proc.stdout
